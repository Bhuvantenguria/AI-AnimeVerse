...
export async function getSelfHostedChapters(mangaId) {
  try {
    const res = await axios.get(`${mangadex_API_BASE}/chapter`, {
      params: {
        manga: mangaId,
...
// 2️⃣ Get CDN panel image URLs from mangadex at-home server
export async function getChapterPages(chapterId) {
  try {
    const res = await axios.get(`${mangadex_API_BASE}/at-home/server/${chapterId}`)
...
export async function getMangaDetails(mangaId) {
  try {
    const res = await axios.get(`${mangadex_API_BASE}/manga/${mangaId}`, {
      params: {
...
export async function searchManga(query, options = {}) {
  try {
    ...
    const res = await axios.get(`${mangadex_API_BASE}/manga`, { params })
...
export async function getTrendingManga(options = {}) {
  try {
    ...
    const res = await axios.get(`${mangadex_API_BASE}/manga`, {
      params: {
        limit: Math.min(limit, 100),
        offset: (page - 1) * limit,
        order: { followedCount: 'desc', rating: 'desc' },
